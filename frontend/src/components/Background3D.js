...
const Background3D = () => {
    ...
    useEffect(() => {
        ...
        // Clear any existing children before appending to avoid duplicates on hot reloads
        ...
        controls.enableZoom = false; // Usually good to disable zoom for a background
        ...
        function transition() {
            ...
            if (mainTitle && statusText && velVal) {
        ...
        transitionInterval = setInterval(transition, 10000);
    ...
    return (
        <div className="relative w-full h-full z-0 overflow-hidden bg-black">
            ...
            {/* Vignette */}
            <div className="absolute inset-0 pointer-events-none" style={{ background: 'radial-gradient(circle, transparent 50%, black 150%)', zIndex: 5 }}></div>
            
            {/* HUD Overlay */}
            <div className="absolute inset-0 pointer-events-none flex flex-col justify-between p-8 md:p-10 z-10" style={{ background: 'radial-gradient(circle at center, transparent 30%, rgba(0,0,0,0.5) 100%)' }}>
                <div className="text-center">
                    <div id="main-title" className="text-xl tracking-[0.8em] uppercase text-white mb-3 font-light opacity-90 transition-opacity duration-700">
                        Stable Singularity
                    </div>
                    <div id="status-text" className="inline-block px-5 py-1.5 bg-white/5 border border-white/15 rounded-full text-[0.65rem] tracking-[0.25em] uppercase transition-all duration-1000 ease-in-out text-[#00f3ff]">
                        Topology: Nominal
                    </div>
                </div>
                
                <div className="flex justify-between items-end font-mono text-[0.7rem] opacity-60 tracking-wider">
                    <div>
                        <div className="mb-1.5">MASS_INDEX: <span className="val text-[#00f3ff] font-bold transition-colors duration-1000">4.2M SOL</span></div>
                        <div className="mb-1.5">LENSING: <span className="val text-[#00f3ff] font-bold transition-colors duration-1000" id="lensing-val">SCHWARZSCHILD</span></div>
                    </div>
                    <div className="text-right">
                        <div className="mb-1.5">RELATIVITY: <span className="val text-[#00f3ff] font-bold transition-colors duration-1000" id="vel-val">0.45c</span></div>
                        <div className="mb-1.5">RADIATION: <span className="val text-[#00f3ff] font-bold transition-colors duration-1000">DETECTION ON</span></div>
                    </div>
                </div>
            </div>
        </div>
